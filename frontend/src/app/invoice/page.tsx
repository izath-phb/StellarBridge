...
export default function InvoicePage() {
  ...
  const totalPending = MOCK_INVOICES.filter((i) => i.status === "PENDING").reduce((s, i) => s + Number(i.amount), 0);
  const totalPaid = MOCK_INVOICES.filter((i) => i.status !== "PENDING").reduce((s, i) => s + Number(i.amount), 0);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Pending Amount", value: `${totalPending} USDC` },
            { label: "Collected", value: `${totalPaid} USDC` },
            { label: "Blockchain Verified", value: `${MOCK_INVOICES.length}` },
          ].map((s, i) => (
              ...
              <CardContent className="px-6 pb-6 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input placeholder="e.g., Acme Corp" className="bg-white/70 border-slate-200 rounded-xl h-11" />
                  </div>
                  <div>
                    <Label className="text-sm font-semibold text-slate-600 mb-2 block">Due Date</Label>
                    <Input type="date" className="bg-white/70 border-slate-200 rounded-xl h-11" />
                  </div>
                  <div>
                    <Label className="text-sm font-semibold text-slate-600 mb-2 block">Amount</Label>
                    <div className="relative">
                      <Input type="number" placeholder="0.00" className="bg-white/70 border-slate-200 rounded-xl h-11 pr-16" />
                  ...
                  <div>
                    ...
                    <Input placeholder="G... Stellar address" className="bg-white/70 border-slate-200 font-mono rounded-xl h-11 text-sm" />
                  </div>
                  <div className="sm:col-span-2">
                    <Label className="text-sm font-semibold text-slate-600 mb-2 block">Description</Label>
                    <Input placeholder="Services rendered..." className="bg-white/70 border-slate-200 rounded-xl h-11" />
                  </div>
                </div>
                ...
                <Button onClick={handleCreate} disabled={creating} className="w-full h-12 rounded-xl font-bold bg-gradient-to-r from-primary to-blue-500 text-white shadow-lg shadow-primary/20">
                  {creating ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Creating on-chain...</> : <><ReceiptText className="w-4 h-4 mr-2" /> Create Tokenized Invoice</>}
                </Button>
              </CardContent>
            </Card>
          </motion.div>
                ...
                <Card className="glass-panel border-0 shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="p-5">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className="text-2xl font-extrabold text-foreground">
                          {Number(inv.amount).toLocaleString()} <span className="text-sm text-slate-400">{inv.asset}</span>
                        </div>
                    ...
                    <div className="mt-4 flex gap-2 border-t border-slate-100 pt-4">
                      {inv.status === "PENDING" && (
                        <Button size="sm" className="rounded-full text-xs bg-gradient-to-r from-primary to-blue-500 text-white">
                          Pay Invoice
                        </Button>
                      )}
